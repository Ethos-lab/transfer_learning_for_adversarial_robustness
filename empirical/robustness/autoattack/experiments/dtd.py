...
class DTD(Dataset):
    def __init__(self, root, split="1", train=False, transform=None, download=False):
        super().__init__()
        download = False #"Download not supported for this dataset !!!"
        ...
        self.samples = [f"{root}/images/{x}" for x in self.ims]
    ...
    def __getitem__(self, index):
        im = Image.open(self.samples[index].strip())
        if self.transform:
