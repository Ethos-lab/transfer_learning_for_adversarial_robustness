...
from .transfer_utils.caltech import Caltech101, Caltech256
from .transfer_utils import aircraft, food_101, dtd
...
def make_loaders_CIFAR10(batch_size, workers, subset):
    ds = ImageNetTransfer(cs.CIFAR10_PATH, num_classes=10, name='cifar10',
                    mean=[0.4914, 0.4822, 0.4465],
                    std=[0.2023, 0.1994, 0.2010])
    ds.custom_class = CIFAR10
...
def make_loaders_CIFAR100(batch_size, workers, subset):
    ds = ImageNetTransfer(cs.CIFAR100_PATH, num_classes=100, name='cifar100',
...
def make_loaders_caltech101(batch_size, workers):
    ds = Caltech101(cs.CALTECH101_PATH, download=False)
    np.random.seed(0)
    ch.manual_seed(0)
...
def make_loaders_caltech256(batch_size, workers):
    ds = Caltech256(cs.CALTECH256_PATH, download=False)
    np.random.seed(0)
    ch.manual_seed(0)
    ch.cuda.manual_seed(0)
    ch.cuda.manual_seed_all(0)
...
def make_loaders(ds, batch_size, workers, subset):
    if ds in ['cifar10', 'cifar100']:
        return TRANSFER_DATASETS[ds](batch_size, workers, subset)

    if subset: raise Exception(f'Subset not supported for the {ds} dataset')
    return TRANSFER_DATASETS[ds](batch_size, workers)
