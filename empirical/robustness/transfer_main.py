...
def main(args, store=None):
    ...
    logger.info(model)
    logger.info(args)
    logger.info(f'Num classes: {ds.num_classes}')
    logger.info(f'Mean/Std: {ds.mean}/{ds.std}')
    print(f"Dataset: {args.dataset} | Model: {args.arch}")

    if args.eval_only:
        result_log = eval_model(args, model, validation_loader, store=store)
        logger.info(result_log)
        sys.exit()

    update_params = freeze_model(model, freeze_level=args.freeze_level)

    train_model(args, model, (train_loader, validation_loader), store=store,
                checkpoint=checkpoint, update_params=update_params, logger=logger)
...
def resume_finetuning_from_checkpoint(args, ds, finetuned_model_path):
    ...
    if args.dataset in list(transfer_datasets.TRANSFER_DATASETS.keys()) and not args.cifar10_cifar10:
        model, _ = make_and_restore_model(
            ...
            dataset=ds, add_custom_forward=args.arch in pytorch_models.keys())
