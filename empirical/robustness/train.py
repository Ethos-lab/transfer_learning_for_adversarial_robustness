...
def make_optimizer_and_schedule(args, model, checkpoint, params):
    ...
    *Internal Function* (called directly from train_model)
    ...
    if args.custom_lr_multiplier == 'cyclic':
        ...
        print('Using custom lr multiplier !!!')
...
def train_model(args, model, loaders, *, checkpoint=None, dp_device_ids=None,
            store=None, update_params=None, disable_no_grad=False, logger=None):
    """
    ...
    lr_update, patience_ctr = (0, 0)
    if checkpoint:
        start_epoch = checkpoint['epoch']
        lr_update, patience_ctr = checkpoint['lr_update'], checkpoint['patience_ctr']
    ...
    for g in opt.param_groups:
        curr_lr = copy.deepcopy(g['lr'])

    epoch = start_epoch
    while lr_update < 3: # allow 2 lr updates
        ...
        def save_checkpoint(filename):
            ckpt_save_path = os.path.join(f'{args.out_dir}/{args.exp_name}' if not store else \
                                          store.path, filename)
        ...
        if logger:
            logger.info(f'epoch: {epoch} time: {epoch_time:.3f} lr: {curr_lr} train_loss: {train_loss:.3f} train_acc: {train_prec1:.1f} test_loss: {nat_loss:.3f} test_acc: {prec1:.1f} adv_loss: {adv_loss:.3f} adv_acc: {adv_prec1:.1f}')

        # lr_update and train stop logic (strict improvement)
        if our_prec1 < best_prec1:
            patience_ctr += 1
        else:
            patience_ctr = 0
        ...
        if patience_ctr == 30:
            ...
            patience_ctr = 0
            if lr_update < 3:
                logger.info(f'lr updated {lr_update} times: {10*curr_lr} to {g["lr"]} (best: {best_prec1:.1f})')
            else:
                logger.info('Terminating training !!!')

        sd_info['lr_update'] = lr_update
        sd_info['patience_ctr'] = patience_ctr

        # If we are at a saving epoch (or the last epoch), save a checkpoint
        if should_save_ckpt: save_checkpoint(ckpt_at_epoch(epoch))
