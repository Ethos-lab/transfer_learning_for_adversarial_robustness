...
from transfer_datasets import DATASETS
from train_utils import AverageMeter, accuracy, requires_grad_, test, prologue
...
parser.add_argument('arch', type=str, choices=['resnet50'])
parser.add_argument('--checkpoint_dir', type=str, default='./checkpoints/consistency')
...
if args.adv_training:
    ...
    mode = f"cohen"
args.outdir = f"{args.checkpoint_dir}/{args.dataset}/{mode}/num_{args.num_noise_vec}/lbd_{args.lbd}/eta_{args.eta}/noise_{args.noise_sd}"
if args.exp_id != '':
    args.outdir += f'/{args.exp_id}'
args.outdir = f'{args.outdir}/{args.arch}'
args.epsilon /= 256.0
args.return_latent = False


def main():
    train_loader, test_loader, criterion, model, optimizer, scheduler, \
    starting_epoch, logger, model_path, writer = prologue(args)
    logger.info(args)
    ...
    for epoch in range(starting_epoch, args.epochs):
        ...
        test_loss, test_acc = test(test_loader, model, criterion, epoch, args.noise_sd, device, writer)
        after = time.time()

        logger.info("{}\t{:.3}\t{:.3}\t{:.3}\t{:.3}\t{:.3}\t{:.3}".format(
            epoch, after - before,
